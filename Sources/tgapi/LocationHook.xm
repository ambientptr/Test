...
@implementation MxFakeLocationManager
...
+ (instancetype)shared {
	...
	static MxFakeLocationManager *instance;
	dispatch_once(&token, ^{
		instance = [[MxFakeLocationManager alloc] init];
	});
	return instance;
}
...
- (id)init {
    self = %orig;
    if (self) {
        [[MxFakeLocationManager shared].locationManagers addObject:self];
    }
    return self;
}
...
- (id)initWithCLLocationManager:(id)locationManager {
    if ([locationManager isKindOfClass:[CLLocationManager class]]) {
        [[MxFakeLocationManager shared].locationManagers addObject:(CLLocationManager *)locationManager];
    }

    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        CLLocationManager *innerLocationManager = [self _clLocationManager];
		[[MxFakeLocationManager shared].locationManagers addObject:innerLocationManager];
    });
