    ...
    enum BotCommand: TypeConstructorDescription {
        ...
        public func serialize(_ buffer: Buffer, _ boxed: Swift.Bool) {
            ...
            case .botCommand(let _data):
                // Mx: 12.9 put a flags word in front of this one, so the
                // legacy form drops the word as well as changing the id.
                if boxed {
                    buffer.appendInt32(MxApiCompat.constructorId(current: -1739401518,
                                                                   legacy: MxApiCompat.legacyBotCommand))
                }
                if MxApiCompat.layer == .l129 {
                    serializeInt32(_data.flags, buffer: buffer, boxed: false)
                }
